FROM confluentinc/cp-kafka-connect:5.5.2-3-ubi8
...
RUN confluent-hub install --no-prompt confluentinc/kafka-connect-activemq:11.0.2
...
RUN confluent-hub install --no-prompt confluentinc/kafka-connect-ibmmq:11.0.2
...
RUN confluent-hub install --no-prompt jcustenborder/kafka-connect-spooldir:2.0.61

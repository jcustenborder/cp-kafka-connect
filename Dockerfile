FROM confluentinc/cp-kafka-connect:6.1.1-2-ubi8
...
RUN confluent-hub install --no-prompt confluentinc/kafka-connect-activemq-sink:1.3.3
...
RUN confluent-hub install --no-prompt confluentinc/kafka-connect-ibmmq-sink:1.3.3
